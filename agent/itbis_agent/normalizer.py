...
def interactive_user() -> tuple[str | None, str | None]:
    """
    (name, domain) of the person signed in at this computer's console.

    Events the agent observes itself (USB drives, removable-drive files,
    downloads) carry no account of their own. The console session's user is
    right even when the agent runs as LocalSystem, where USERNAME would name
    the machine account instead. Falls back to the agent's own environment.
    """
    try:
        import win32ts  # type: ignore[import-not-found]

        session = win32ts.WTSGetActiveConsoleSessionId()
        if session != 0xFFFFFFFF:  # no one at the console
            server = win32ts.WTS_CURRENT_SERVER_HANDLE
            name = win32ts.WTSQuerySessionInformation(server, session, win32ts.WTSUserName)
            if name:
                domain = win32ts.WTSQuerySessionInformation(
                    server, session, win32ts.WTSDomainName
                )
                return name, domain or None
    except Exception:  # noqa: BLE001 - not Windows, or the WTS query failed
        pass
    return os.environ.get("USERNAME"), os.environ.get("USERDOMAIN")


def session_user(session_id: int) -> tuple[str | None, str | None]:
    """(name, domain) of whoever is signed in to a Windows session, if anyone."""
    try:
        import win32ts  # type: ignore[import-not-found]

        server = win32ts.WTS_CURRENT_SERVER_HANDLE
        name = win32ts.WTSQuerySessionInformation(server, session_id, win32ts.WTSUserName)
        if name:
            domain = win32ts.WTSQuerySessionInformation(
                server, session_id, win32ts.WTSDomainName
            )
            return name, domain or None
    except Exception:  # noqa: BLE001 - not Windows, or the session is gone
        pass
    return None, None
...
class Normaliser:
    ...
    def _group_membership(self, raw: dict, s: list) -> CanonicalEvent | None:
        ...
        # A group is not a person: keep Windows' spelling (Builtin\Administrators).
        group_name, group_domain = _field(s, 2) or group_sid or "unknown", _field(s, 3)
    ...
    def _from_process(self, raw: dict) -> CanonicalEvent | None:
        ...
        if not user and session is not None:
            # Short-lived commands often exit before their owner can be looked
            # up, but Windows recorded the session they started in: session 0
            # runs services, any other belongs to whoever is signed in to it.
            if int(session) == 0:
                if not self._keep_system:
                    return None
                user = "NT AUTHORITY\\SYSTEM"
            else:
                name, domain = session_user(int(session))
                if name:
                    user = self._format_user(name, domain)
                    tags.append("owner_from_session")
        if not user:
    ...
    @staticmethod
    def _format_user(username: str | None, domain: str | None = None) -> str:
        """
        One spelling per account, so a person gets one behavioural profile.

        Windows account names are case-insensitive and each source spells
        them differently (security log, process owner, session query), which
        split one person into several users. The form is DOMAIN\\name with
        the domain upper-cased and a person's name lower-cased. Built-in
        service accounts keep Windows' spelling (NT AUTHORITY\\SYSTEM).
        """
        username = (username or "").strip()
        if not username:
            return "unknown"
        if "@" in username:  # user principal name, e.g. jsmith@corp.example
            return username.lower()
        if "\\" in username:
            domain, _, username = username.partition("\\")
        domain = (domain or "").strip().upper() or None
        if not is_service_account(username, domain):
            username = username.lower()
        return f"{domain}\\{username}" if domain else username

    def _console_user(self) -> str:
        name, domain = interactive_user()
        return self._format_user(name, domain)
