...
def event_time_utc(time_generated) -> str | None:
    """
    An event record's TimeGenerated as an ISO-8601 UTC string.

    pywin32 returns it as a naive datetime in the host's local time. Sending
    that as-is stored events off by the host's UTC offset and with no timezone
    at all, which later crashed feature generation on the server.
    """
    if not time_generated:
        return None
    return time_generated.astimezone(UTC).isoformat()
            ...
            while True:
                ...
                for ev in events:
                    ...
                    yield {
                        ...
                        "time_generated": event_time_utc(ev.TimeGenerated),
